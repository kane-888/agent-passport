...
import {
  ...
  buildWorkingMemoryCheckpoint as buildWorkingMemoryCheckpointImpl,
  compactConversationToPassportMemories,
  extractExplicitPreferencesFromText,
  summarizePassportMemoryWrites,
  writeExplicitPreferenceMemories as writeExplicitPreferenceMemoriesImpl,
} from "./ledger-passport-memory-writes.js";
import {
  runPassportOfflineReplayCycle as runPassportOfflineReplayCycleImpl,
...
const writeExplicitPreferenceMemories = (store, agent, explicitPreferences, options = {}) =>
  writeExplicitPreferenceMemoriesImpl(store, agent, explicitPreferences, options, buildPassportMemoryWriteDeps());

const buildWorkingMemoryCheckpoint = (store, agent, options = {}) =>
  buildWorkingMemoryCheckpointImpl(store, agent, options, buildPassportMemoryWriteDeps());

function buildPassportOfflineReplayDeps() {
  return {
    appendEvent,
  };
}

const runPassportOfflineReplayCycle = (store, agent, options = {}) =>
  runPassportOfflineReplayCycleImpl(store, agent, options, buildPassportOfflineReplayDeps());

function buildPassportMemoryMaintenanceDeps() {
  return {
    appendEvent,
...
const runPassportMemoryMaintenanceCycle = (store, agent, options = {}) =>
  runPassportMemoryMaintenanceCycleImpl(store, agent, options, buildPassportMemoryMaintenanceDeps());
