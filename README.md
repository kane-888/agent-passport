...
公开运行态 `/` 和修复中心现在共享 `public/ui-links.js` 里的入口命名约定；当前公开名是 `AgentPassportLinks`，旧 `OpenNeedRuntimeLinks` 只作为兼容别名保留。修复中心仍然保留自己的 deep-link 状态，但 `/` 本身不再承诺消费 repair / credential 上下文。
