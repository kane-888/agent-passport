...
当前原型的身份、记忆、恢复和记忆稳态运行链已经进入同一套 `agent-passport` 主仓库。后续继续往下接：

1. 继续把 `ledger.js` 里剩余的记忆稳态接入代码拆到专用 integration seam。
2. 收紧记忆稳态 prompt preflight / formal execution receipts 的默认门禁。
3. 再接真正的链上 DID / 钱包地址。
4. 扩展 Verifiable Credentials、多签授权撤销与时间锁。
5. 补 Agent 信誉分与可校验履历。
