...
- `npm run smoke:dom`
  ...
  当前会覆盖主控制台 / 修复中心 deep-link、agentId / didMethod / windowId 主视角恢复、窗口绑定 / 引用窗口视图、repairId / credentialId / repair 分页 round-trip、status list selector / compare selector 状态保留、compare 参数改写，以及 sibling method 切换后的状态 / 时间线一致性
- `npm run smoke:ui`
...
这个页面是从主控制台里拆出来的干净修复中枢，专门用于查看 repair 列表、分页、受影响 credential、credential 深钻、状态证明和 repair 时间线。
...
主控制台和修复中心现在共享 `public/ui-links.js` 里的 deep-link 契约；这意味着 agent 可以直接复用同一套 query 规则来生成、恢复和校验页面上下文。

### `GET /`

主控制台现在也支持深链 query 参数，适合从修复中心或别的 agent 结果里直接跳回当前上下文：
