...
公开运行态 `/` 和修复中心现在共享 `public/ui-links.js` 里的 deep-link 契约；这意味着 agent 可以直接复用同一套 query 规则来生成、恢复和校验页面上下文。

### `GET /`

公开运行态首页。它不再承载旧 dashboard，只回答 3 件事：服务是否活着、正式恢复是否仍在窗口内、自动恢复有没有越过 operator boundary。
