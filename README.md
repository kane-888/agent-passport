...
`agent-passport` 是对外产品名。`OpenNeed 记忆稳态引擎` 是为 `agent-passport` 提供本地真值、记忆稳态、恢复与审计能力的底层引擎。
...
- `npm run smoke:dom`
  ...
  当前会覆盖公开运行态入口命名约定、`buildPublicRuntimeHref()` 固定回 `/`、runtime-home query helper 的参数 round-trip、修复中枢 deep-link 契约、窗口绑定 / 引用窗口视图、repairId / credentialId / repair 分页 round-trip、status list selector / compare selector 状态保留，以及 sibling method 切换后的状态 / 时间线一致性
- `npm run smoke:ui`
...
公开运行态 `/` 和修复中枢现在共享 `public/ui-links.js` 里的入口命名约定；当前公开名是 `AgentPassportLinks`，旧 `OpenNeedRuntimeLinks` 只作为兼容别名保留。修复中枢仍然保留自己的 deep-link 状态，但 `/` 本身不再承诺消费 repair / credential 上下文。
...
另外当前会同步一份 `config_only` 的并行 subagent 真值：

- `parallelSubagentPolicy`：主控闸门、最大并行数、放行条件、阻塞条件
- `subagentPlan`：每个线程角色的批次、激活阶段、依赖关系、写入范围、并行候选关系

这层目前先解决“线程有没有同步到真正可读的并行 subagent 配置”问题，还不会自动 fan-out 执行子 Agent。
