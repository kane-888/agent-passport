...
- `npm run smoke:dom`
  ...
  当前会覆盖公开运行态 / 修复中心 deep-link 契约、agentId / didMethod / windowId 参数保真、窗口绑定 / 引用窗口视图、repairId / credentialId / repair 分页 round-trip、status list selector / compare selector 状态保留、compare 参数改写，以及 sibling method 切换后的状态 / 时间线一致性
- `npm run smoke:ui`
...
- 这条浏览器级回归默认使用 Safari
- 这条回归会等到 `/` 真正进入“公开运行态已加载”成功态；如果首页落到读取失败文案，会直接失败
...
公开运行态 `/` 和修复中心现在共享 `public/ui-links.js` 里的 deep-link 契约；当前公开名是 `OpenNeedRuntimeLinks`，旧 `AgentPassportLinks` 只作为兼容别名保留。这意味着 agent 可以直接复用同一套 query 规则来生成、恢复和校验页面上下文。

### `GET /`

公开运行态首页。它不再承载旧混合控制台，只回答 3 件事：服务是否活着、正式恢复是否仍在窗口内、自动恢复有没有越过 operator boundary。

它仍然保留深链 query 参数，主要用于从修复中心回跳时保留上下文，以及给共享 `ui-links` 契约和浏览器回归做 round-trip 校验：
...
如果 URL 带了这些参数，首页会保留它们并维持回跳链路，但不会重新展开旧的 Agent 主视角、证据区或状态列表面板。深操作现在固定留在：
