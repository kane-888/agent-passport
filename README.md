...
- 发布机器可读的 Agent Passport 产品 / 接口元描述
...
- `npm run smoke:dom`
  ...
  当前会覆盖主控制台 / Repair Hub deep-link、agentId / didMethod / windowId 主视角恢复、窗口绑定 / 引用窗口视图、repairId / credentialId / repair 分页 round-trip、status list selector / compare selector 状态保留、compare 参数改写，以及 sibling method 切换后的状态 / 时间线一致性
- `npm run smoke:ui`
...
- 如果这项没有开启，脚本会明确提示这是本机 Safari 设置问题，不会误报成项目代码故障

默认地址：

- Web UI: `http://localhost:4319`
- Repair Hub: `http://localhost:4319/repair-hub`
...
把当前本地的 store key / signing master secret 迁移到系统 Keychain。
...
```json
{
  ...
  "title": "Agent Passport Bootstrap",
...
这个页面是从主控制台里拆出来的干净 repair 中枢，专门用于查看 repair 列表、分页、受影响 credential、credential 深钻、状态证明和 repair 时间线。
...
其中 `credentialId` 可以配合 sibling method 切换使用，让 Repair Hub 直接落到某个并行 DID method 的 credential 视角。

主控制台和 Repair Hub 现在共享 `public/ui-links.js` 里的 deep-link 契约；这意味着 agent 可以直接复用同一套 query 规则来生成、恢复和校验页面上下文。

### `GET /`

主控制台现在也支持深链 query 参数，适合从 Repair Hub 或别的 agent 结果里直接跳回当前上下文：
...
- `compareLeftAgentId`
- `compareRightAgentId`
- `compareIssuerAgentId`
- `compareIssuerDidMethod`

如果 URL 带了 `agentId` / `didMethod` / `windowId` / `repairId` / `credentialId` / `statusListId` / `statusListCompareId` / `repairLimit` / `repairOffset`，主控制台会自动恢复当前 Agent 主视角、DID 视角、窗口上下文引用、repair 上下文、证据区视角，以及状态列表浏览器 / 对比面板 / repair 分页的选中状态。

主控制台的共享中枢面板现在还会单独显示：

- 当前本地窗口绑定
- 当前引用窗口绑定
- 当前 dashboard 视角与引用窗口是否一致
- 从引用窗口一键切回当前窗口，或按引用窗口切到对应 Agent 视角
