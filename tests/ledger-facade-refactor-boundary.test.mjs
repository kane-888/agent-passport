...
test("passport memory replay helpers stay outside ledger facade", () => {
  for (const functionName of [
    ...
    "buildPassportMemoryReplayGroups",
    "buildPassportReplaySummary",
    "scorePassportOfflineReplayPriority",
    "buildPassportReplayGroupDrivers",
    "shouldRunPassportOfflineReplay",
    "buildPassportOfflineReplayEventGraph",
    "buildPassportSleepStageTrace",
    "buildPassportOfflineReplayNarrative",
    ...
    assert.doesNotMatch(
      ledgerSource,
      new RegExp(`\\n(?:export\\s+)?function ${functionName}\\s*\\(`),
      `${functionName} should remain in src/ledger-passport-memory-replay.js`
    );
    assert.match(
      passportMemoryReplaySource,
      new RegExp(`export function ${functionName}\\s*\\(`),
      `${functionName} must be exported by src/ledger-passport-memory-replay.js`
    );
  }
  assert.doesNotMatch(
    passportMemoryReplaySource,
    /from "\.\/ledger\.js";/,
    "src/ledger-passport-memory-replay.js must not import the ledger facade"
  );
});

test("ledger offline replay facade calls use explicit dependency injection", () => {
  assert.match(
    ledgerSource,
...
test("runtime record normalizers stay outside ledger facade", () => {
  for (const functionName of [
    ...
    assert.doesNotMatch(
      ledgerSource,
      new RegExp(`\\n(?:export\\s+)?function ${functionName}\\s*\\(`),
      `${functionName} should remain in src/ledger-runtime-records.js`
    );
    assert.match(
...
test("runtime search helper shapes stay outside ledger facade", () => {
  for (const functionName of [
    ...
    assert.doesNotMatch(
      ledgerSource,
      new RegExp(`\\n(?:export\\s+)?function ${functionName}\\s*\\(`),
      `${functionName} should remain in src/ledger-runtime-search.js`
    );
    assert.match(
      runtimeSearchSource,
      new RegExp(`export function ${functionName}\\s*\\(`),
      `${functionName} must be exported by src/ledger-runtime-search.js`
    );
  }
});

test("context prompt view helpers stay outside ledger facade", () => {
  for (const functionName of [
    ...
    assert.doesNotMatch(
      ledgerSource,
      new RegExp(`\\n(?:export\\s+)?function ${functionName}\\s*\\(`),
      `${functionName} should remain in src/ledger-context-prompt-views.js`
    );
    assert.match(
...
test("cognitive state helpers stay outside ledger facade", () => {
  for (const functionName of [
    ...
    "inferCognitiveMode",
    "inferCognitiveDominantStage",
    "extractStablePreferences",
    "extractPreferenceSignalsFromText",
    "deriveCognitivePreferenceProfile",
    ...
    assert.doesNotMatch(
      ledgerSource,
      new RegExp(`\\n(?:export\\s+)?function ${functionName}\\s*\\(`),
      `${functionName} should remain in src/ledger-cognitive-state.js`
    );
    assert.match(
      cognitiveStateSource,
      new RegExp(`export function ${functionName}\\s*\\(`),
      `${functionName} must be exported by src/ledger-cognitive-state.js`
    );
  }
});

test("ledger cognitive state facade calls use explicit dependency injection", () => {
  const callPatterns = [
    /resolveEffectiveAgentCognitiveState\s*\([^;]+buildCognitiveStateDeps\(\)\s*\)/gs,
    /buildContinuousCognitiveState\s*\([^;]+buildCognitiveStateDeps\(\)\s*\)/gs,
  ];
  for (const pattern of callPatterns) {
    const matchedCalls = ledgerSource.match(pattern) || [];
    assert.equal(matchedCalls.length > 0, true, `${pattern} must match at least one facade call`);
  }
  ...
  for (const functionName of [
    ...
    const calls = ledgerSource.match(new RegExp(`${functionName}\\s*\\(`, "g")) || [];
    const injectedCalls = ledgerSource.match(
      new RegExp(`${functionName}\\s*\\([^;]+buildCognitiveStateDeps\\(\\)\\s*\\)`, "gs")
    ) || [];
    assert.equal(
      injectedCalls.length,
      calls.length,
