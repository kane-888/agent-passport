...
const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "agent-passport-memory-context-demo-"));
const ledgerPath = path.join(tempDir, "ledger.json");

process.env.AGENT_PASSPORT_LEDGER_PATH = ledgerPath;
process.env.AGENT_PASSPORT_CHAIN_ID = "agent-passport-memory-context-demo";
...
async function main() {
  ...
  const bootstrap = await bootstrapAgentRuntime(
    agent.agentId,
    {
      displayName: "沈知远",
      role: "CEO",
      longTermGoal: "长期目标是稳定推进 agent-passport 记忆稳态 runtime",
  ...
  const llmServer = http.createServer((req, res) => {
    ...
    req.on("end", () => {
      ...
      const taskFrameText = taskFrameMatch?.[1] || "";
      let taskFrame = null;
      try {
        taskFrame = JSON.parse(taskFrameText);
      } catch {
        taskFrame = null;
      }
