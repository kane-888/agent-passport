...
function buildFallbackResponse(input = {}) {
  ...
  return {
    responseText: [
      ...
      .filter(Boolean)
      .join("\n"),
    model: MEMORY_STABILITY_LOCAL_REASONER_LABEL,
    provider: "fallback_local_reasoner",
    strategy: "memory_stability_engine_local_first",
  };
}
      ...
      messages: [
        {
          role: "system",
          content:
            "You are the memory-stability engine local reasoner serving agent-passport. Ground every reply in the supplied identity and memory context. Keep the output concise.",
        },
        {
          role: "user",
          content: prompt,
...
async function main() {
  ...
  const input = raw
    ...
    : {
        contextBuilder: {
          compiledPrompt: "identitySnapshot: agent_passport_memory_demo / role=memory-runtime-assistant",
          slots: {
            identitySnapshot: {
              agentId: "agent_passport_memory_demo",
              profile: {
                name: "Memory Stability Engine Assistant",
        ...
        payload: {
          currentGoal: "验证记忆稳态引擎本地 reasoner 接口",
  ...
  const payload = input?.payload || {};
  const prompt = [
    "Use the supplied memory-stability-engine context as the grounding reference for local reasoning. Identity, long-term memory, recovery, and audit truth still belong to agent-passport.",
